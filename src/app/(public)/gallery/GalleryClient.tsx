...
export default function GalleryClient({ dbMedia }: GalleryClientProps) {
  const staticItems = generateGalleryItems();
  
  // Format dynamic media files from DB settings
  const dynamicItems: GalleryItem[] = dbMedia.map((m) => ({
    src: m.url,
    category: "Uploads",
    alt: m.name
  }));

  // Database uploads are prepended for instant visual feedback
  const allItems = [...dynamicItems, ...staticItems];

  const [filter, setFilter] = useState("All");
  const [lightboxIndex, setLightboxIndex] = useState<number | null>(null);

  const categories = ["All", "Uploads", "Founders", "Outreach", "Churches", "Relief", "Seminars", "Women", "Baptisms", "Bible Stories"];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
        ...
        <div className="text-center max-w-3xl mx-auto space-y-4">
          ...
          <p className="text-lg text-slate-500 dark:text-slate-400 font-light leading-relaxed">
            Browse through authentic photos illustrating our church planting, leadership training, women's seminars, river baptisms, and disaster relief work across Nepal.
          </p>
        </div>
            ...
            <button
              ...
              className={`px-4 py-2 rounded-full text-xs font-bold transition shadow-sm cursor-pointer ${
                filter === cat
                  ...
                  : "bg-white dark:bg-slate-955 text-slate-605 dark:text-slate-400 border border-slate-100 dark:border-slate-800/80 hover:bg-slate-100 hover:text-slate-900 dark:hover:text-white"
              }`}
            >
              {cat}
            </button>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {filteredItems.map((item, idx) => (
            <div
              ...
              className="group aspect-square relative rounded-2xl overflow-hidden shadow-md border border-slate-202/40 dark:border-slate-800/40 cursor-pointer bg-slate-200 dark:bg-slate-950"
            >
              <Image
                src={item.src}
                alt={item.alt}
